...
const unsigned char font5x7 [][CHAR_WIDTH - 1] PROGMEM = {
...
const unsigned char menu_icons [][MENU_WIDTH] PROGMEM = 
{
    { 0x38, 0x44, 0x82, 0x82, 0xBA, 0xA2, 0x82, 0x44, 0x38},   // Clock
    { 0x00, 0x08, 0x88, 0xC8, 0x7E, 0x7E, 0xC8, 0x88, 0x08},   // Person
    { 0x38, 0xC4, 0x9A, 0xD2, 0x82, 0xD2, 0x9A, 0xC4, 0x38},   // Purge
    { 0x00, 0xC0, 0x7C, 0x73, 0x7C, 0xC0, 0x00, 0x00, 0x00}    // Spacecraft
};
    ...
    )
{
    ...
    int contrast = 0x40;
    lcd_send(0x21, LCD_CMD);                // LCD Extended Commands
    lcd_send(0x85 | contrast, LCD_CMD);     // Set LCD Vop(Contrast)
...
{  
    ...
    for(int i = start; i < end; i++) lcd_send(0, LCD_DATA);
}
...
// Displays menu item
void LCD_5110::lcd_menu(unsigned char menu)
{
    lcd_base_addr(lcdCacheIdx);

    // 5 pixel wide characters and add space
    for(unsigned char i=0; i < MENU_WIDTH; i++) 
        lcd_send(pgm_read_byte(&menu_icons[menu][i]), LCD_DATA);
}
...
void LCD_5110::printPictureOnLCD_nonpgm (unsigned char *data)
{
    int pixel_cols = LCD_WIDTH * (LCD_HEIGHT / CHAR_HEIGHT); //<6> means 6 lines on LCD.
    lcd_goto_xy(0, 0);
    //lcd_send(0x08, LCD_CMD);
